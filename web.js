...
window.addEventListener('appinstalled', () => {
    ...
    pwaInstallBtn.style.display = 'none';
    showNotification('3nity Hair app installed!');
});
...
function displayProducts(productsToDisplay) {
    ...
    productsToDisplay.forEach(product => {
        ...
        // Attach quick view listener to the entire card (except add to cart button)
        ...
        // Attach quick view listener to the button as well
...
function updateCart() {
    ...
    cart.forEach(item => {
        ...
        // Make cart item clickable to open quick view
        cartItem.addEventListener('click', function (e) {
            // Don't open quick view if clicking on controls
            if (!e.target.classList.contains('qty-btn') &&
                !e.target.classList.contains('qty-input') &&
                !e.target.classList.contains('remove-btn')) {
                openQuickView(item.id);
                closeCart(); // Close cart modal when opening quick view
            }
        });
    });
...
function loadCartFromStorage() {
    const stored = localStorage.getItem('indexCart');
    if (stored) {
